...
$(document).ready(() => {
   ...
   const brands =  $('.brand');
    const firstIndexBrand = brands.eq(2)

    const firstPrice = $('.price:first');
    firstPrice.css('color',"red")
    ...
    cards.addClass('round')
    // cards.removeClass('round')

    

    console.log(firstIndexBrand)

    $('#my_button').click(function(e){
        console.log('click 2')
        lastPrice.slideToggle()
    })
...
const createCard = (product)=>{
   const cardTemplate =  getCardTemplate(product);
   const card = $(cardTemplate); // dom element object
   $(".products").append(card); // finding body in dom and return body dom object, appending card into body dom element  
}

const getCardTemplate = (productInfo) => {
    const {preview,name,brand,price} = productInfo||{};
  return `
    <div class="card">
...
function createH2() {
    const h2 = $('<h2 class="heading"> Hello Its very simple</h2>');
    $("body").append(h2);
  }
...
//     h2.innerHTML = "Hello Its very simple"
...
const str = `My Name is ${name} ${lname}`
console.log(str)